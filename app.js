'use strict';
const SK = 'sout_v1_';
const PAGE_ID = (location.pathname.split('/').pop() || 'index').replace('.html', '');
...
function loadAll() {
  document.querySelectorAll('.tselect').forEach(el => {
    ...
    if (v !== null) { el.value = v; el.classList.toggle('filled', !!v); }
    const pv = el.parentElement.querySelector('.print-val');
    if (pv) pv.textContent = el.value || '';
  });
}
...
/* ── CSV helpers ─────────────────────────────────────────────────── */
function buildCSV() {
  const rows = ['id,valeur'];
  document.querySelectorAll('.tselect').forEach(el => {
    rows.push(el.id + ',' + el.value);
  });
  return rows.join('\r\n');
}
...
async function getWritableHandle() {
  let handle = await idbGet(PAGE_ID);
  if (handle) {
  ...
  handle = await window.showSaveFilePicker({
    id: 'sout_save',
    suggestedName: PAGE_ID + '_enseignants.csv',
    types: [{ description: 'Fichier CSV', accept: { 'text/csv': ['.csv'] } }]
  });
  await idbPut(PAGE_ID, handle);
  return handle;
}
...
function ghFilePath() {
  return `data/${PAGE_ID}_enseignants.csv`;
}
...
async function saveToGitHub() {
  ...
  const body = { message: `Mise à jour ${PAGE_ID} — ${new Date().toLocaleString('fr-FR')}`, content, branch };
  if (sha) body.sha = sha;
...
function injectGHUI() {
  ...
  if (nav) {
    ...
    btn.className = 'gh-btn' + (isGHConfigured() ? ' gh-btn--active' : '');
    btn.id = 'gh-config-btn';
    btn.title = 'Configuration GitHub';
    btn.innerHTML = `<svg viewBox="0 0 16 16" fill="currentColor" width="15" height="15"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0 0 16 8c0-4.42-3.58-8-8-8z"/></svg>GitHub`;
    const pdfBtn = nav.querySelector('.pdf-btn');
    if (pdfBtn) nav.insertBefore(btn, pdfBtn);
    else nav.appendChild(btn);
    btn.addEventListener('click', openGHModal);
  }
...
<div class="gh-modal">
  ...
  <div class="gh-modal-body">
    ...
    <div class="gh-form-group">
      <label class="gh-label">Token <span class="gh-hint">(GitHub → Settings → Developer settings → PAT)</span></label>
...
function clearGHConfig() {
  ...
  ['gh-token','gh-owner','gh-repo'].forEach(id => { document.getElementById(id).value = ''; });
  document.getElementById('gh-branch').value = 'main';
...
async function testGHFromModal() {
  ...
  showGHStatus(
    ok ? '✓ Connexion réussie !' : '✗ Échec — vérifiez le token et les noms (owner/repo).',
    ok ? 'success' : 'error'
  );
}
...
async function saveSelectionsToFile() {
  ...
  if (window.showSaveFilePicker) {
    try {
      ...
      if (e.name !== 'AbortError') downloadCSV(csv);
    }
  } else {
    downloadCSV(csv);
  }
...
function downloadCSV(csv) {
  ...
  a.download = PAGE_ID + '_enseignants.csv';
...
async function autoLoadFromFile() {
  ...
  const handle = await idbGet(PAGE_ID);
  if (!handle) return;
  try {
